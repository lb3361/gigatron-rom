...
static void load_book(char *filename)
{
        ...
        booksize = 0;
#ifdef LOAD_BOOK_BIN
	fp = fopen(BOOKNAME, "rb");
	if (fp) {
		booksize = fread(&core, sizeof(struct bk), CORE, fp);
		fclose(fp);
		if (booksize > 0) {
			printf("Loading opening book of size %d from '" BOOKNAME "'\n", booksize);
			return;
		}
	}
#endif
...
# endif
        printf("Dumping binary book into " BOOKNAME "\n");
	fp = fopen(BOOKNAME,"wb");
