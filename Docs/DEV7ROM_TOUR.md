...
LATER CHANGES: In more recent versions of DEV7ROM, variables `ledTempo` and `ledTimer` have been [eliminated](https://forum.gigatron.io/viewtopic.php?p=4740#p4740). The sequencer state is not entirely stored in variable `ledState_v2` in a manner that remain compatible with the official way to stop or restart the led sequencer. 
...
* The [most complex stub](https://github.com/lb3361/gigatron-rom/blob/doc/Core/dev.asm.py#L7954-L7969) stores two argument into `sysArgs[5]` and `sysArgs[6]` before firing the state machine. This is only used by three opcodes, [`MOVL`](https://github.com/lb3361/gigatron-rom/blob/doc/Core/dev.asm.py#L8022), [`MOVF`](https://github.com/lb3361/gigatron-rom/blob/doc/Core/dev.asm.py#L8030), and [`COPYS`](https://github.com/lb3361/gigatron-rom/blob/doc/Core/dev.asm.py#L7567), implemented in fsm page 24.
* Instruction [`MULW`](https://github.com/lb3361/gigatron-rom/blob/doc/Core/dev.asm.py#L7774) uses a stub that collects a single argument into `sysArgs+6`. Single-argument stubs are [implemented](https://github.com/lb3361/gigatron-rom/blob/doc/Core/dev.asm.py#L7924-L7927) with an [indirection](https://github.com/lb3361/gigatron-rom/blob/doc/Core/dev.asm.py#L7911-L7921) in order to share code. This was possible because there was one cycle left to obtain the required even number of cycles.
...
* Fragment [`copy#3b`](https://github.com/lb3361/gigatron-rom/blob/doc/Core/dev.asm.py#L8153-L8186) copies a single byte and increments both the source pointer `vT3` and the destination pointer `vT2`. If it encounters a page boundary, it schedules fragment `copy#3c`. Otherwise it returns to the dispatcher. The next fragment will then be either `copy#3a` (when copying the final four bytes, fragment `copy#3a` jumps into `copy#3b`, leaving `fsmState` set to `copy#3a`) or `copy#3b` (otherwise).
* Fragment [`copy#3c`](https://github.com/lb3361/gigatron-rom/blob/doc/Core/dev.asm.py#L8188-L8214) fixes the source and destination pointers to deal with page crossings and schedules `copy#3a` again.
...
LATER CHANGES: in more recent versions of DEV7ROM, all this code has been [reorganized in a more systematic way](https://forum.gigatron.io/viewtopic.php?t=566), saving a couple cycles for vCPU opcode execution. 
...
LATER CHANGES: in more recent versions of DEV7ROM, all this code has been modified, [saving a couple cycles for vCPU opcode execution](https://forum.gigatron.io/viewtopic.php?t=566). 
